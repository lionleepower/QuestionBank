...
## 2026-07-02：`rm -- *.err` 里的 `--` 是文件名占位符吗？

**我的问题**

我想删除当前目录下所有 `.err` 文件，看到命令：

```bash
rm -- *.err
```

这里的 `--` 是不是类似占位符，表示后面是文件名称？

**我的理解 / 回答**

一开始以为 `--` 可能是一个占位符，用来表示文件名的位置。

**批改**

`--` 不是占位符，也不是文件名。它是很多 Unix/Linux 命令支持的“选项结束标记”。
...
这里 `--` 本身不会被删除，它只是告诉 `rm` 后面的 `a.err`、`b.err` 是文件名。

它最有用的场景是文件名以 `-` 开头。例如有个文件叫：
...
这样 `rm` 就知道 `-force.err` 是文件名。

**记住**

```text
-- 不是占位符，而是“选项结束标记”。
rm -- *.err 表示：删除 *.err 展开后的文件，并且后面的名字都按文件名处理。
删除前不确定时，先用 ls -- *.err 或 find . -name '*.err' -type f 预览。
```
